...
choicefilelabel = Label(text="STEP1: SELECT IMAGE FILE".ljust(14), font=("Arial", 24))# 標示按鈕功能
choicefilelabel.grid(row=0, column=0)#標示按鈕位置 choicefilelabel.grid(row=0, column=0)#標示按鈕位置
markoptionlabel = Label(text="STEP2: EDIT WATERMARK".ljust(8), font=("Arial", 24))
...
def editbutton_action():
    ...
    setting_window.minsize(width=230, height=400) #調整視窗大小
    # TODO 設定頁面的滾動條 步驟
    ...
    def print_and_get():
        # text_for_watermark = entrytext.get()
        # text_for_watermark = entrytext.get() #用Enrtry()元件時獲得輸入內容
        text_for_watermark = entrytext.get("1.0", "end-1c") #用text()元件，END會取的空格所以改end-1c https://stackoverflow.com/questions/14824163/how-to-get-the-input-from-the-tkinter-text-widget
...
#---------設定配置-----------------------------------------------------------------------------------------------------------
    ...
    # entrytext = Entry(second_frame, width=15)  # 輸入的元件，但無法多行輸入
    # entrytext.insert(INSERT, "key in Text")  # 可以輸入END和INSERT但不知道差別在哪
    
    #換成text widget 才可以輸入多行文字 參考:https://steam.oxxostudio.tw/category/python/tkinter/text.html
    entrytext = Text(second_frame, height=3, width=15)
    ...
    shrinkagelabel.grid(column=1, row=0)
    ...
    Label(second_frame, text="Position").grid(column=1, row=2)
    ...
    # saveImagebutton = Button(second_frame, text="SAVE",command=saveImage, width=10, height =2)
    # saveImagebutton.grid(row=0, column=1)
    # #TODO新增SAVE按鈕測試存檔後圖示，測試到底為什麼浮水印字體為什麼不能調整透明
    saveImagebutton = Button(second_frame, text="SAVE", command=saveImage, width=10, height=3)  # 注意command的function都不能有()，會變成布須觸發就執行
    saveImagebutton.grid(row=6, column=1)

    edit_finish_button = Button(second_frame, text="PREVIEW", command=print_and_get, width=10, height=3, bg= "green" ,fg="yellow")  # 注意command的function都不能有()，會變成布須觸發就執行
    edit_finish_button.grid(row=5, column=1)

   

openImagebutton = Button(text="Import", command=button1_action, width=5, font=('Arial', 12 ,"bold"))
openImagebutton.grid(row=0, column=1) #選擇檔案按鈕

editImagebutton = Button(text='Option', command=editbutton_action, width=5, font=('Arial', 12 ,"bold"))
