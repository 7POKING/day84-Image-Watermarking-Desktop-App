...
Font = ImageFont.truetype(os.path.join(fontsFolder, 'arial.ttf'), 32)#Window系統路
...
def editbutton_action():
    ...
    def print_and_get():
        ...
        if baseImage != "None Photo":
            ...
            print("not save s%" %(textImg))
            # textImg.save(Desktop/'withtext.png')
            print("Save Done?")
            # textImg.show()
            # return os.path.basename(textImg)
            # baseImage = textImg
            # return baseImage
        else:
            print("None Photo")
...
def saveImage():
    pass
    # print(editbutton_action)
    # editbutton_action().save(os.path.join('withtext', baseImage))
